...
namespace GhpAPI.Services
{
    public class ReportService
    {
        ...
        public async Task<DailyReportDto> GetDailyReport(int schoolId, DateTime start, DateTime end)
        {
            var rows = await (
        ...
        public async Task<List<FormReportRowDto>> GetGhpReport(int schoolId, DateTime start, DateTime end)
        {
            var rows = await (
                from i in _db.Inspections
                join ind in _db.InspectionDetails on i.Id equals ind.InspectionId
        ...
        public async Task<List<FormReportRowDto>> GetVisitingReport(int schoolId, DateTime start, DateTime end)
        {
            var rows = await (
                from i in _db.Inspections
                join ind in _db.InspectionDetails on i.Id equals ind.InspectionId
        ...
        public async Task<CheckValueReportDto> GetProdTempReport(int schoolId, DateTime start, DateTime end)
        {
            ...
            foreach (var row in rows)
            {
                ...
                {
                    rowMap[date] = new Dictionary<string, object?> { { "date", row.Date } };
                }
                if (rowMap[date].ContainsKey(prodAttrMap[row.No])) continue;
                if (row.Status != "pass")
                {
                    abnormalRows.Add(new AbnormalRowDto
                    {
                ...
                rowMap[date][prodAttrMap[row.No]] = row.No == "8-01" ? row.Status == "pass" : row.CheckValue;
            }

            return new CheckValueReportDto
            {
                Rows = rowMap.Values.ToList(),
                AbnormalRows = abnormalRows,
            };
        ...
        public async Task<CheckValueReportDto> GetTnhReport(int schoolId, DateTime start, DateTime end)
        {
            ...
            foreach (var row in rows)
            {
                ...
                if (rowMap[date].ContainsKey(tnhAttrMap[row.No])) continue;
                if (row.Status != "pass")
                {
                    abnormalRows.Add(new AbnormalRowDto
                    {
        ...
        public async Task<CheckValueReportDto> GetTablewareReport(int schoolId, DateTime start, DateTime end)
        {
            ...
            foreach (var row in rows)
            {
                ...
                if (rowMap[date].ContainsKey(tablewareAttrMap[row.No])) continue;
                if (row.Status != "pass")
                {
                    abnormalRows.Add(new AbnormalRowDto
                    {
